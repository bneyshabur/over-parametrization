...
# trainingi the model for one epoch on the given set
def train(args, model, device, train_loader, criterion, optimizer, epoch):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        # computer the classification error and loss
...
# evaluating the model on the given set
def validate(args, model, device, val_loader, criterion):
    ...
    return 1 - (sum_correct / len(val_loader.dataset)), sum_loss / len(val_loader.dataset), np.percentile( margin.cpu().numpy(), 5 )
...
def load_data( split, dataset, datadir, nchannels):
    ...
    tr_transform = transforms.Compose([transforms.RandomCrop(32, padding=4), transforms.RandomHorizontalFlip(), transforms.ToTensor(), normalize])
...
def main():
    ...
    # create model
    model = torch.nn.Sequential(torch.nn.Linear(32 * 32 * nchannels, args.nunits), torch.nn.ReLU(), torch.nn.Linear(args.nunits, nclasses))
    ...
    for epoch in range(0, args.epochs):
        ...
        print(f'Epoch: {epoch + 1}/{args.epochs}\t Training loss: {tr_loss:.3f}\t Training error: {tr_err:.3f}\t Validation error: {val_err:.3f}')

        # stop training if the cross-entropy loss is less than the stoping condition
    ...
    print(f'\nFinal: Training loss: {tr_loss:.3f}\t Training margin {tr_margin:.3f}\t Training error: {tr_err:.3f}\t Validation error: {val_err:.3f}\n')
